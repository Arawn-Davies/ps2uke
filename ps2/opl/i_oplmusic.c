...
#include "opl_shim.h"   // byte/boolean, SHORT, DEH_String, M_*, Z_* (ps2uke)

#include "opl.h"
#include "midifile.h"
#include "ps2_opl.h"    // public PS2OPL_* API exposed at the bottom of this file

// The GENMIDI (GM->OPL instrument bank) is embedded rather than loaded from a
// WAD lump: ps2uke has no "genmidi" lump, so we link in the DMX demo bank
// (demo_genmidi.c). It begins with the "#OPL_II#" header, like a real lump.
extern unsigned char demo_genmidi[];

// Output sample rate of the OPL software synth, set by PS2OPL_Init() to match
// the audsrv mixer rate (replaces Chocolate Doom's snd_samplerate global).
static unsigned int opl_mix_rate = 44100;
...
static boolean LoadInstrumentTable(void)
{
    ...
    lump = (byte *) demo_genmidi;    // embedded GENMIDI bank (ps2uke)
...
static void I_OPL_ShutdownMusic(void)
{
    if (music_initialized)
    {
        ...
        // (GENMIDI is a statically-linked array; nothing to release.)
...
static boolean I_OPL_InitMusic(void)
{
    ...
    OPL_SetSampleRate(opl_mix_rate);
...
//----------------------------------------------------------------------
//
// ps2uke public API (ps2_opl.h). Duke's sounds.c drives these directly,
// bypassing jfaudiolib's MIDI sequencer; the audsrv feed thread pulls audio
// via OPL_PS2_Render() (declared in opl_ps2.c). Duke's MusicVolume is 0..255,
// the OPL sequencer wants 0..127.
//
//----------------------------------------------------------------------

static void *ps2opl_song = NULL;   // currently-registered MIDI handle
...
void PS2OPL_Init(int samplerate)
{
    ...
    if (samplerate > 0)
    {
        opl_mix_rate = (unsigned int) samplerate;
    }
...
void PS2OPL_PlaySong(void *data, int len, int looping)
{
    ...
    PS2OPL_StopSong();
    ps2opl_song = I_OPL_RegisterSong(data, len);
    if (ps2opl_song != NULL)
    {
        I_OPL_PlaySong(ps2opl_song, looping ? true : false);
    }
}
...
void PS2OPL_StopSong(void)
{
    ...
    if (ps2opl_song != NULL)
    {
        I_OPL_UnRegisterSong(ps2opl_song);
        ps2opl_song = NULL;
    }
}

void PS2OPL_SetVolume(int duke_volume)   // 0..255 -> 0..127
{
    int v = (duke_volume * 127) / 255;
    if (v < 0)   v = 0;
    if (v > 127) v = 127;
    I_OPL_SetMusicVolume(v);
}
...
{
    return I_OPL_MusicIsPlaying() ? 1 : 0;
}
